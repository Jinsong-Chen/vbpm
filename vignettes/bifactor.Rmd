...
```{r step1-reading}
## Analysis-side count profile, not a package rule: ELBO, 20%, sustain one,
## strict crossing after the last maximum gain, full look-ahead, no fallback.
read_count <- function(x, cut = .20) {
  K <- x$sweep$K
  score <- x$sweep$ELBO
  ...
  hit <- which(seq_along(gain) >= first_after_peak & gain < cut * gain_max)
  K_hat <- if (length(hit)) K[hit[1L]] else NA_integer_
  c(count_path_usable = TRUE, K_hat = K_hat)
}

## One local persistence profile: minimum absolute phi >= .80, direct horizon
## r = 2, sources 2:3, and a collision veto on every required edge.
read_persistence <- function(x, sources = 2:3, cutoff = .80, horizon = 2L) {
  phi <- x$persistence$phi
  collision <- x$persistence$collision
  state <- vapply(sources, function(k) {
    targets <- k + seq_len(horizon)
    if (!all(as.character(k) %in% rownames(phi)) ||
        !all(as.character(targets) %in% colnames(phi)))
      return(NA)
    value <- phi[as.character(k), as.character(targets)]
    clash <- collision[as.character(k), as.character(targets)]
    if (anyNA(value) || anyNA(clash)) NA else
      all(value >= cutoff & !clash)
  }, logical(1))

  persistent <- which(state %in% TRUE)
  if (!length(persistent)) {
    resolution <- if (all(state %in% FALSE)) "absent" else "unresolved"
    K_p <- NA_integer_
  } else {
    top <- max(persistent)
    higher <- seq.int(top + 1L, length(state))
    resolved <- top == length(state) || all(state[higher] %in% FALSE)
    resolution <- if (resolved) "selected" else "unresolved"
    K_p <- if (resolved) sources[top] else NA_integer_
  }
  list(K_p = K_p, resolution = resolution,
       source_state = setNames(state, sources))
}

read_profile <- function(x) {
  count <- read_count(x)
  persistence <- read_persistence(x)
  usable <- as.logical(count[["count_path_usable"]])
  K_hat <- as.integer(count[["K_hat"]])
  K_p <- persistence$K_p
  layer <- if (identical(persistence$resolution, "selected") && usable &&
               is.finite(K_hat) && identical(K_hat, K_p)) {
    "L1"
  } else if (identical(persistence$resolution, "selected") && usable) {
    "L2"
  } else if (identical(persistence$resolution, "absent") && usable) {
    "L3"
  } else {
    "unclassified"
  }
  c(K_hat = K_hat, K_p = K_p, layer = layer)
}

profile <- rbind(AO = read_profile(AO), AZ = read_profile(AZ))
profile
```

Both count paths return three, and the displayed `.80/r2` profile also selects
three in both backbones, so both readings are L1 here. That agreement is a fact
about this sample, source set, horizon, cutoff, and window. The AIC minimum at
four is in the same evidence and disagrees, and no part of `pefa()` arbitrates
between them. The profile is tutorial analysis code: `.80/r2` was retained as a
scenario-bound practical default in one simulation study, not made a `vbpm`
default. Shallower `.85/r1` and deeper `.70/r3` readings, or matched
maximum-RMSD companions, remain legitimate declared sensitivity profiles.

A horizon is always direct: $r=2$ asks separately for `K -> K + 1` and
`K -> K + 2`, never for a chain through the intervening fit. Horizons one
through four are constructed the same way, but source `K` requires the fitted
endpoint `K + r`. A known collision vetoes that edge for this profile. If every
declared source resolves nonpersistent the result is L3; unavailable evidence
that prevents resolution is unclassified, not L3.

## Step 2: a comparison derived from Step 1

The criterion path supplies the descriptive count $\widehat K$. The persistence
...
```{r step2-design}
if (!profile["AO", "layer"] %in% c("L1", "L2"))
  stop("The declared profile did not deliver a Step-2 proposal.")
K_step2 <- as.integer(profile["AO", "K_p"])
K0      <- ncol(AO$Q0)
